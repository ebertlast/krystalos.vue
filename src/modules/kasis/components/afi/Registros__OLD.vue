    ...
    >
      ...
      <template slot="detalles">
        <v-container grid-list-md text-xs-center v-if="crud===VER">
          ...
          <v-card v-else class="pt-4 mx-auto" flat max-width="1000">
            <v-card-text>
              <!-- <v-avatar size="88">
                <v-img :src="avatar" class="mb-4"></v-img>
              </v-avatar>-->
              <h3 class="headline mb-2">{{nombre_paciente}}</h3>
              <div class="blue--text mb-2">{{afi.EMAIL}}</div>
              <div
                class="blue--text subheading font-weight-bold"
              >{{afi.TIPO_DOC}} - {{afi.DOCIDAFILIADO}}</div>
            </v-card-text>
            <v-divider></v-divider>
            <v-layout tag="v-card-text" text-xs-left wrap>
              ...
              <v-flex xs2 class="text-xs-left">{{ afi.TELEFONOLAB || "Sin Información" }}</v-flex>
              <v-flex tag="strong" xs4 class="text-xs-right">Teléfono Particular:</v-flex>
              <v-flex xs2 class="text-xs-left">{{ afi.TELEFONORES || "Sin Información" }}</v-flex>
            </v-layout>
            <v-divider></v-divider>
      ...
      <template slot="formulario">
        <formulario
          :afi="afi"
          @guardar="registrarAfi($event)"
          @cancelar="$refs.tabla.dialogFormulario = false;"
          ref="formulario_ingreso"
        ></formulario>
...
export default {
  ...
  computed: {
    ...
    nombre_paciente() {
      return `${this.afi.PAPELLIDO} ${this.afi.SAPELLIDO || ""} ${
        this.afi.PNOMBRE
      } ${this.afi.SNOMBRE || ""}`;
    },
    avatar() {
