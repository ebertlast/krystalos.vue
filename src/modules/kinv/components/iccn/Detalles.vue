        ...
        <v-hover>
          <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 12 : 2}`" class="mx-auto">
            <v-card-text class="text-sm-left">
              ...
              <div>{{(tipoArticulo) ? tipoArticulo : ""}}</div>
            </v-card-text>
          </v-card>
        </v-hover>
      </v-flex>
...
export default {
  ...
  methods: {
    ...
    refrescarDatos() {
      if (!this.fila || !this.fila.IDCONCENTRA) return;
      this.cargando = true;
      this.model = {};
      this.$http
        .get(`iccn/${this.fila.IDCONCENTRA}`)
        .then(res => {
          this.model = res.result.recordset[0];
        })
        .then(() => {
          this.$http
            .get(`itar/${this.model.IDITAR}`)
            .then(res => {
              this.tipoArticulo = res.result.recordset[0].DESCRIPCION;
            })
            .then(() => {
              this.cargando = false;
            })
