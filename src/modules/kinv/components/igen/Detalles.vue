    ...
    <v-layout row wrap>
      ...
      <v-flex xs3>
        <v-hover>
          <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 12 : 2}`" class="mx-auto">
            <v-card-text class="text-sm-left">
              <span class="grey--text">IDTGENERICO</span>
              <div>{{(model && model.IDTGENERICO) ? model.IDTGENERICO : ""}}</div>
            </v-card-text>
          </v-card>
        </v-hover>
      </v-flex>
...
export default {
  ...
  methods: {
    ...
    refrescarDatos() {
      if (!this.fila || !this.fila.IDGENERICO) return;
      this.cargando = true;
      this.model = {};
      this.$http
        .get(`igen/${this.fila.IDGENERICO}`)
        .then(res => {
