        ...
        <v-hover>
          <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 12 : 2}`" class="mx-auto">
            <v-card-text class="text-sm-left">
              <span class="grey--text">IDARCHIVO</span>
              <div>{{(model && model.IDARCHIVO) ? model.IDARCHIVO : ""}}</div>
            </v-card-text>
          </v-card>
        </v-hover>
      </v-flex>
...
export default {
  ...
  methods: {
    ...
    refrescarDatos() {
      if (!this.fila || !this.fila.IDARCHIVO) return;
      this.cargando = true;
      this.model = {};
      this.$http
        .get(`auta_bot/${this.fila.IDARCHIVO}`)
        .then(res => {
