        ...
        <v-hover>
          <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 12 : 2}`" class="mx-auto">
            <v-card-text class="text-sm-left">
              <span class="grey--text">IDCATEGORIA</span>
              <div>{{(model && model.IDCATEGORIA) ? model.IDCATEGORIA : ""}}</div>
            </v-card-text>
          </v-card>
        </v-hover>
      </v-flex>
...
export default {
  ...
  methods: {
    ...
    refrescarDatos() {
      if (!this.fila || !this.fila.IDCATEGORIA) return;
      this.cargando = true;
      this.model = {};
      this.$http
        .get(`cat/${this.fila.IDCATEGORIA}`)
        .then(res => {
