    ...
    <v-layout row wrap>
      ...
      <v-flex xs3>
        <v-hover>
          <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 12 : 2}`" class="mx-auto">
            <v-card-text class="text-sm-left">
              <span class="grey--text">DESCENTRAL</span>
              <div>{{(model && model.DESCENTRAL) ? model.DESCENTRAL : ""}}</div>
            </v-card-text>
          </v-card>
        </v-hover>
      </v-flex>
      <v-flex xs3>
        <v-hover>
          <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 12 : 2}`" class="mx-auto">
            <v-card-text class="text-sm-left">
              <span class="grey--text">RETICA</span>
              <div>{{(model && model.RETICA) ? model.RETICA : ""}}</div>
            </v-card-text>
          </v-card>
        </v-hover>
      </v-flex>
      <v-flex xs3>
        <v-hover>
          <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 12 : 2}`" class="mx-auto">
            <v-card-text class="text-sm-left">
              <span class="grey--text">FACTORICA</span>
              <div>{{(model && model.FACTORICA) ? model.FACTORICA : ""}}</div>
            </v-card-text>
          </v-card>
        </v-hover>
      </v-flex>
      <v-flex xs3>
        <v-hover>
          <v-card slot-scope="{ hover }" :class="`elevation-${hover ? 12 : 2}`" class="mx-auto">
            <v-card-text class="text-sm-left">
              <span class="grey--text">VLRGESTION</span>
              <div>{{(model && model.VLRGESTION) ? model.VLRGESTION : ""}}</div>
            </v-card-text>
          </v-card>
        </v-hover>
      </v-flex>
...
export default {
  ...
  data: () => ({
    ...
    model: {
      ...
      DPTO: undefined,
      NOMBRE: undefined,
      DESCENTRAL: undefined,
      RETICA: undefined,
      FACTORICA: undefined,
      VLRGESTION: undefined
    }
  }),
  mounted() {
    this.refrescarDatos();
  ...
  methods: {
    ...
    refrescarDatos() {
      if (!this.fila || !this.fila.CIUDAD) return;
      this.cargando = true;
      this.model = {};
      this.$http
        .get(`ciu/${this.fila.CIUDAD}`)
        .then(res => {
